...
abstract public class GraphView extends LinearLayout {
	static final String TAG="jjoie";
	static final private class GraphViewConfig {
		static final float BORDER = 15;//julian 20
	...
	static public class GraphViewData {
		...
		public final short valueT;//julian
		...
		public GraphViewData(double valueX, double valueY,short t) {
			super();
			this.valueX = valueX;
			this.valueY = valueY;
			this.valueT=t;
			//this.GraphViewData(valueX,valueY);
			//GraphViewData(0,0);
		...
		if (viewportStart == 0 && viewportSize == 0) {
			...
			for (int i=0; i<values.length; i++) {
				...
				if (values[i].valueX >= viewportStart) {
					...
					Log.e(TAG,"values");
					//listData.add(new GraphViewData(0, 10)  );
				}
			}
	...
	private double getMaxX(boolean ignoreViewport) {
		...
		if (!ignoreViewport && viewportSize != 0) {
			...
			if(values.length>0){
				...
				try{
			 highest = values[values.length-1].valueX; //crash
			for (int i=1; i<graphSeries.size(); i++) {
				values = graphSeries.get(i).values;
				highest = Math.max(highest, values[values.length-1].valueX);
			}
			
				}catch(Exception e){Log.e(TAG,"getmaxx");
				//highest=200;
				double largest = Integer.MIN_VALUE;
				for (int i=0; i<graphSeries.size(); i++) {
					GraphViewData[] valuesk = _values(i);
					for (int ii=0; ii<valuesk.length; ii++)
						if (valuesk[ii].valueX > largest)
							largest = valuesk[ii].valueX;
				}
				Log.e(TAG,"getmaxxk ");
				highest=largest;
				}
