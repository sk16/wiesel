...
public class AntPlusManager {
    ...
    /** The Log Tag. */
    public static final String TAG = "JuliANT";
    ...
    /** ANT+ channel period for an HRM 8070*///probably here 4hz
    static final short HRM_PERIOD = 10070;//4hz 6000 5hz
    //julian
    private short dHRM_PERIOD=10070;
 ...
 // Variables to keep track of the status of each sensor
    ...
    private int mBPM = 0;
    private int [ ] amBPM ={0,0,0,0,0,0,0};
 ...
 //   private Timer timer=new Timer();
    ...
    /** counter how much resets are gone */
    ...
    public boolean start()
    {
        ...
        //timerforrunningcheck();
        
        return initialised;
    }
  /*  
    private void timerforrunningcheck(){
    ...
    */
    ...
    public void switchpauseapp(){
    	Log.d(TAG,"switch");
    	if(brunning)pauseapp();
    	else resumeapp();
    }
    ...
    private void WriteSettings(Context context, String data){
 	  ...
 	  String sdom="";
 	   ...
 	   File file = new File(sdCard.getAbsolutePath() + "/bpm/log" +sdom +(new Integer(month)).toString()+ sdoy + (new Integer(dom)).toString() + ".txt");
 	   try {
    ...
    public void setHRMPeriod(short b){
    	
    	this.dHRM_PERIOD=b;
    }
    public short getHRMPeriod(){
    	
    	return this.dHRM_PERIOD;
    }
    ...
    public int getaverageBPM(int count){
    	
    	return (amBPM[0]+amBPM[1]+amBPM[2])/3;
    }
        ...
        if (!deferToNextReset)
        {
            ...
            switch (channel)
            {
                case HRM_CHANNEL:
                    ...
                    period = dHRM_PERIOD;//TODO get the period from preferences
                    Log.d(TAG,"hrm period");
    ...
    public void requestReset()
    {
        ...
        {
            mAntResetSent = true;
            mAntReceiver.ANTResetSystem(); //TODO crash here at beginning
            setAntConfiguration();
        } catch (AntInterfaceException e) {
    ...
    private void waitandreset(){
       ...
       /* handler.postDelayed(new Runnable() { 
            public void run() { 
           	 closeChannel(AntPlusManager.HRM_CHANNEL);
           	 
           	openChannel(AntPlusManager.HRM_CHANNEL, true);
           	requestReset();
           	Log.d(TAG, "reset2");
            } 
       }, 180*1000); */
        handler.postDelayed(new Runnable() { 
            public void run() { 
            	if(resetcounter>0)
            	resetantnow();
           	Log.d(TAG, "reset3 "+(new Integer(resetcounter)).toString());
            } 
       }, 7*60*1000); 
        handler.postDelayed(new Runnable() { 
            public void run() { 
            	if(resetcounter>0)
            	resetantnow();
           	Log.d(TAG, "reset2"+(new Integer(resetcounter)).toString());
            } 
       }, 30*60*1000); 
    //	}
    }
    
    public void resetantnow(){
    	Calendar cal = Calendar.getInstance();
  	   int min = cal.get(Calendar.MINUTE);
  	   int hour = cal.get(Calendar.HOUR_OF_DAY);
    	...
    	openChannel(AntPlusManager.HRM_CHANNEL, true);
    	 requestReset();
    	 WriteSettings(mActivity.getApplicationContext(), hourn +(new Integer(hour)).toString() +minn+ (new Integer(min)).toString() + " reset" + "\n");
    }
    ...
    {      
               ...
               case AntDefine.EVENT_RX_SEARCH_TIMEOUT:
                   switch(ANTRxMessage[AntMesg.MESG_DATA_OFFSET]) //Switch on channel number
                   {
                       case HRM_CHANNEL:
                           ...
                           //julian wait 2min and reset ant
                           waitandreset();
                           break;
                       case SDM_CHANNEL:
                           try
       ...
       private void antDecodeWeight(byte[] ANTRxMessage)
       {
          ...
          if(mAntProcessingEnabled)
          {
             ...
             if(mDeviceNumberWGT == ANTPlusDemoj.WILDCARD)
             {
                 try
                 {
       ...
       private void antDecodeSDM(byte[] ANTRxMessage)
       {
          ...
          if(mAntProcessingEnabled)
          {
             ...
             if(mDeviceNumberSDM == ANTPlusDemoj.WILDCARD)
             {
                 try
                 {
       ...
       private void antDecodeHRM(byte[] ANTRxMessage)
       {
          ...
          if(mAntProcessingEnabled)
          {
             ...
             if(mDeviceNumberHRM == ANTPlusDemoj.WILDCARD)
             {
                 try
                 {
          ...
          savebpm();
          Log.d(TAG, "antDecodeHRM end " + (new Integer(mBPM)).toString());
       }
    };
    ...
    public void savebpm(){
 	...
 	/*   Calendar cal = Calendar.getInstance();
        ...
        if (hour<10)hourn="0";
        if (min<10)minn="0";*/
       // WriteSettings(mActivity.getApplicationContext(), hourn +(new Integer(hour)).toString() +minn+ (new Integer(min)).toString() + " "+(new Integer(getaverageBPM(3))).toString() + "\n");
    ...
    private boolean antChannelSetup(byte networkNumber, byte channelNumber, short deviceNumber, byte deviceType, byte txType, short channelPeriod, byte radioFreq, byte proxSearch)
    {
       ...
       try
       {
           ...
           if(deviceNumber == ANTPlusDemoj.WILDCARD)
           {
